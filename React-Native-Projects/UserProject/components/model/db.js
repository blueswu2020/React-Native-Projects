...
//根据教育活动id查找所有已报名的用户
app.get('/educationEnrolledUsers/:activityId',function(req,res){
    conn.query('SELECT userPhone FROM education_enroll WHERE activityId=?',req.params.activityId,function(error,rows,fields){
        if(!!error) console.log('error');
        else {
            console.log(rows);
            res.send(JSON.stringify(rows));
...
//根据教育活动id查找所有已报名的用户
app.get('/employmentEnrolledUsers/:activityId',function(req,res){
    conn.query('SELECT userPhone FROM employment_enroll WHERE activityId=?',req.params.activityId,function(error,rows,fields){
        if(!!error) console.log('error');
        else {
            console.log(rows);
            res.send(JSON.stringify(rows));
