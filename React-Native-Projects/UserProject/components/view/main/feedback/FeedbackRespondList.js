...
export default  class FeedbackResponse extends Component{
    constructor(props) {
        super(props);
        ...
        this.state={
            ...
            text:null,//搜索关键字
            renderList:null,//显示列表
            respondList:null,//回复列表
        }
    }
        ...
        return (
            <TouchableOpacity activeOpacity={0.5} onPress={() => this.showDetail(item.item)}>
                <View style={styles.itemContainer}>
                    <Text style={styles.itemTextTitle}>关于您的反馈"{item.item.context.slice(0,5)}"有了回复...</Text>
                </View>
            </TouchableOpacity>
        );
    }
    ...
    // 关键词过滤
    filterText = (text) => {
        let data = this.state.respondList
        if (text.length > 0) {
    ...
    onGetRespondList = () => {
        storage.load({
            ...
            fetch('http://192.168.1.3:3000/feedbackRespondList/' + ret,{  //获取该管理员管理的用户列表
                method: 'GET'
            }).then((response) => {
                return response.json();
            }).then((jsonData) => {
                let json1 = jsonSort(jsonData,'time',true); //根据发布日期对取回的json数组进行排序
                this.setState({
                    respondList: json1,
                    renderList: json1,
                })
                //console.log(this.state.respondList);
            })
                .catch((error) => {
                    console.warn(error);
                }).done();
    ...
    //跳转到活动详情页
    showDetail(item) {
        this.props.navigation.navigate('FeedbackRespondDetail',
            {
                id: item.id,
            });
    }
