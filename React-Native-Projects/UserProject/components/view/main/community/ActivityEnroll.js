...
export default class ActivityEnroll extends Component {
    ...
    // 关键词过滤
    filterText = (text) => {
        let data = this.state.activityList
        if (text.length > 0) {
    ...
    //跳转到活动详情页
    showDetail(item) {
        this.props.navigation.navigate('ActivityEnrollDetail',
            {
                id: item.id,
            });
    }
    ...
    onGetActivitiesId = () => {
        return new Promise((resolve,reject) => {
            storage.load({
                ...
                // 如果找到数据，则在then方法中返回
                fetch('http://192.168.1.3:3000/activityEnrolls/' + ret ,{
                    method: 'GET'
                }).then((response) => {
    ...
    onGetActivityList = () => {
        return new Promise((resolve, reject) => {
            this.onGetActivitiesId()
                .then((idList) => {
                    if(1 === idList.length){
                        fetch('http://192.168.1.3:3000/activity/' + idList[0].activityId, {  //根据活动id获取活动信息
                            method: 'GET'
                        }).then((response) => {
                            return response.json();
                        }).then((jsonData) => {
                            this.setState({
                                activityList: jsonData,
                                renderList: jsonData
                            })
                        })
                            .catch((error) => {
                        ...
                        idList.map((item,index) =>
                            fetch('http://192.168.1.3:3000/activity/' + item.activityId, {  //根据活动id获取活动信息
                                method: 'GET'
                            }).then((response) => {
                                return response.json();
                            }).then((jsonData) => {
                                this.state.activityList.push(jsonData[0]);
    ...
    //根据发布日期排序
    // timeSort = () => {
    //     let json1 = jsonSort(this.state.activityList,'releaseTime',true); //根据发布日期对取回的json数组进行排序
    //     this.setState({
    //         activityList: json1,
    //         renderList: json1,
    //     })
