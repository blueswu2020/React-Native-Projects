...
export default class ActivityDetail extends Component {
    ...
    //设置定时通知
    onSetScheduleNotify = () => {
        let getTime = this.state.activity.startTime; //获取活动开始时间
    ...
    //获取活动报名信息
    onGetEnroll = () => {
        fetch('http://192.168.1.3:3000/activityEnroll/' + this.state.id, {  //根据活动id查找报名用户是否存在
            method: 'GET'
        }).then((response) => {
    ...
    //新增活动报名信息
    onPostActivityEnroll = () => {
        fetch('http:192.168.1.3:3000/activityEnroll', {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({activityId: this.state.id, userPhone: this.state.phone})
        }).then((response) => {
            return response.json();
        }).then((jsonData) => {
            this.setState({
    ...
    //修改活动信息
    onUpdateActivity = () => {
        fetch('http://192.168.1.3:3000/activityEnrollNumber/',{
            method: 'PUT',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
    ...
    onJoinInHandler = () => {
        //如果人数已满的话就提示人数已满
        if(this.state.activity.maxNumber > this.state.enrollNumber){
            ...
            this.onPostActivityEnroll();
            this.onUpdateActivity();
            ToastAndroid.show("已报名,活动开始前一小时自动提醒，请放心", ToastAndroid.SHORT);
        }else{
