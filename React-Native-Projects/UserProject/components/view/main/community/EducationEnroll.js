...
export default class EducationEnroll extends Component {
    constructor(props) {
        super(props);
        this.state = {
            userInfo: '',
    ...
    componentDidMount() {
        this.onGetEducationList(); //获取活动列表
    }
    ...
    //跳转到活动详情页
    showDetail(item) {
        this.props.navigation.navigate('EducationEnrollDetail',
            {
                id: item.id,
            });
    }
    ...
    _onRefresh() {
        ...
        let json1 = jsonSort(this.state.educationList,'releaseTime',true); //根据发布日期对取回的json数组进行排序
        this.state.educationList = json1;
    ...
    onGetEducationsId = () => {
        return new Promise((resolve,reject) => {
            storage.load({
                ...
                // 如果找到数据，则在then方法中返回
                fetch('http://192.168.1.3:3000/educationEnrolls/' + ret ,{
                    method: 'GET'
                }).then((response) => {
    ...
    //获取活动列表
    onGetEducationList = () => {
        this.onGetEducationsId()
            .then((idList)=>{
                if(1 === idList.length){
                    fetch('http://192.168.1.3:3000/education/' + idList[0].educationId, {  //根据活动id获取活动信息
                        method: 'GET'
                    }).then((response) => {
                        return response.json();
                    }).then((jsonData) => {
                        this.setState({
                            educationList: jsonData,
                            renderList: jsonData
                        })
                    })
                        .catch((error) => {
                    ...
                    for(let i = 0; i < idList.length; i++ ){
                        fetch('http://192.168.1.3:3000/education/' + idList[i].educationId, {  //根据活动id获取活动信息
                            method: 'GET'
                        }).then((response) => {
                            return response.json();
                        }).then((jsonData) => {
                            this.state.educationList.push(jsonData[0]);
