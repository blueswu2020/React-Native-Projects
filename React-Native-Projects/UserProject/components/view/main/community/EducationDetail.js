...
export default class EducationDetail extends Component {
    ...
    //设置定时通知
    onSetScheduleNotify = () => {
        let getTime = this.state.education.startTime; //获取活动开始时间
    ...
    //获取活动报名信息
    onGetEnroll = () => {
        fetch('http://192.168.1.3:3000/educationEnroll/' + this.state.id, {  //根据活动id查找报名用户是否存在
            method: 'GET'
        }).then((response) => {
    ...
    //新增活动报名信息
    onPostEducationEnroll = () => {
        fetch('http:192.168.1.3:3000/educationEnroll', {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({educationId: this.state.id, userPhone: this.state.phone})
        }).then((response) => {
            return response.json();
        }).then((jsonData) => {
            this.setState({
    ...
    //修改活动信息
    onUpdateEducation = () => {
        fetch('http://192.168.1.3:3000/educationEnrollNumber/',{
            method: 'PUT',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
    ...
    onJoinInHandler = () => {
        //如果人数已满的话就提示人数已满
        if(this.state.education.maxNumber > this.state.enrollNumber){
            this.state.enrollNumber += 1;
            this.setState({
                text: '已报名',
                touchDisabled : true,
            })
            this.onPostEducationEnroll();
