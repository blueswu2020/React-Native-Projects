...
export default class ActivityEdit extends Component {
    ...
    UNSAFE_componentWillMount() {
        ...
        this.setState({
            ...
            enrollNumber: activity.enrollNumber, //修改活动之后将报名人数清零
    ...
    onConfirmHandler = () => {
        if (
            this.state.title === this.state.activity.title
            && this.state.context === this.state.activity.context
            && this.state.location === this.state.activity.location
            && this.state.startTime === this.state.activity.startTime
            && this.state.endTime === this.state.activity.endTime
            && this.state.maxNumber === this.state.activity.maxNumber
            && this.state.attention === this.state.activity.attention
        ){
            Alert.alert('提示：','未修改任何内容')
        }else{
            this.onUpdateActivity();
            this.turnToActivityDetail();
        }
    }

    //修改活动信息
    onUpdateActivity = () => {
        fetch('http://192.168.1.3:3000/activity/',{
            method: 'PUT',
            headers: {
